jest.mock("@/modules/invoice/invoice.factory", () => {
  ...
  const { GetOneInvoiceService } = jest.requireActual(
  ...
  return {
    buildCreateInvoiceModule: () => {
      const repoMock = {
        findById: jest.fn().mockRejectedValue(new Error("Repo failure")),
      } as unknown as InvoiceRepository;

      const createService = { execute: jest.fn() } as any;
      const getAllInvoicesService = { execute: jest.fn() } as any;
      const getOneInvoiceService = new GetOneInvoiceService(repoMock);
...
describe("GET /invoice/:id - 500 when repository throws", () => {
  beforeAll(async () => {
    await init();
  });
